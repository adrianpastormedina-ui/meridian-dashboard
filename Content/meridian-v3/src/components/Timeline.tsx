...
export default function Timeline({ language }: TimelineProps) {
  const [activeGrade, setActiveGrade] = React.useState<string>('9° Grado');
  const t = DICTIONARY[language];

  const grades = ['9° Grado', '10° Grado', '11° Grado', '12° Grado'];

  // Filter milestones based on grade
  const currentMilestones = TIMELINE_DATA.filter((m) => m.grade === activeGrade);
  ...
  return (
    <section id="metodologia" className="py-24 bg-[#070B19] relative overflow-hidden scroll-mt-10 border-t border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Dynamic Grade Selector Tabs (9º - 12º) */}
        ...
        {/* Expanded Chronological Milestone Stage Area */}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {currentMilestones.map((milestone) => (
                ...
                >
                  {/* Category Visual Core Icon */}
                  <div className={`p-4 rounded-2xl border shrink-0 ${getCategoryColor(milestone.category)}`}>
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Checklists for each focus step */}
